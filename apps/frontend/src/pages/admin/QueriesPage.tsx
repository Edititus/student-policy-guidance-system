...
const QueriesPage: React.FC<IQueriesPageProps> = ({
  ...
  const counts = useMemo(
    () => ({
      all: queries.length,
      pending: queries.filter((q) => getStatus(q) === 'pending' || getStatus(q) === 'in_review')
        .length,
  ...
  const handleRespond = async () => {
    ...
    try {
      await onRespond(modal.query.queryId, adminResponse)
      closeModal()
    } catch {
  ...
  const handleDismiss = async () => {
    ...
    setActionLoading(modal.query.queryId)
    try {
      await onDismiss(modal.query.queryId)
      closeModal()
    } catch {
  ...
  const handleDelete = async () => {
    ...
    setActionLoading(modal.query.queryId)
    try {
      await onDelete(modal.query.queryId)
      closeModal()
    } catch {
    ...
    <>
      <div className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate mt-0.5">
              Queries flagged for admin review due to low AI confidence
            </p>
          </div>
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-primary w-full"
            />
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <div className="flex gap-1 bg-smoke rounded-xl p-1 w-fit min-w-max">
            {TABS.map(({ key, label }) => (
              <button
                key={key}
              ...
              <div className="p-12 text-center">
                ...
                <div className="text-base font-medium text-teal-deep">
                  {search
                    ...
                    : `No ${activeTab === 'all' ? '' : activeTab} queries`}
                </div>
                {activeTab === 'pending' && !search && (
                  <div className="text-sm text-slate mt-1">
                    All escalated queries have been handled!
                  </div>
                )}
              </div>
            ) : (
                    ...
                    <div className="flex items-start gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          ...
                          <span
                            className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-medium border ${sm.color}`}
                          >
                        ...
                        <p className="text-xs text-slate bg-smoke rounded-lg px-3 py-2 line-clamp-2 mb-2">
                          <span className="font-medium text-teal-deep">AI: </span>
                          {query.aiAnswer}
                        </p>
                        ...
                        {status === 'pending' || status === 'in_review' ? (
                          <>
                            <Button
                              variant="primary"
                              size="sm"
                              onClick={() => setModal({ type: 'respond', query })}
                            >
                              Respond
                            </Button>
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => setModal({ type: 'dismiss', query })}
                            >
                              Dismiss
                            </Button>
                          </>
                          ...
                          <>
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => setModal({ type: 'view', query })}
                            >
                              View
                            </Button>
                            <Button
            ...
            <div className="p-6 space-y-4">
              {modal.query.studentName && (
                <p className="text-xs text-slate flex items-center gap-1">
                  ...
                  {modal.query.studentEmail && (
                    <span className="text-slate/60">· {modal.query.studentEmail}</span>
                  )}
                </p>
              )}
              <div>
                <label className="text-sm font-medium text-teal-deep block mb-1.5">
                  Student's Question
                </label>
                <div className="p-3 bg-smoke rounded-xl text-sm text-teal-deep">
              ...
              <div>
                ...
                <div className="p-3 bg-red-50 rounded-xl text-sm text-teal-deep border border-red-100">
                  {modal.query.aiAnswer}
                </div>
              </div>
              <div>
                <label className="text-sm font-medium text-teal-deep block mb-1.5">
                ...
                <p className="text-xs text-slate/60 mt-1">
                  This response will be emailed to the student and embedded in the AI knowledge
                  base.
                </p>
              </div>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              {modal.query.respondedAt && (
                <p className="text-xs text-slate/60">
                  Responded {new Date(modal.query.respondedAt).toLocaleString()}
                </p>
              )}
            </div>
            ...
            <div className="flex items-start gap-4 mb-5">
              ...
              <div>
                ...
                <p className="text-sm text-slate mt-1">
                  The query will be marked as dismissed. The student will <strong>not</strong>{' '}
                  receive a response. You can still delete it afterwards.
                </p>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl p-6">
            <div className="flex items-start gap-4 mb-5">
              ...
              <div>
                ...
                <p className="text-sm text-slate mt-1">
                  This will <strong>permanently remove</strong> the record. This action cannot be
                  undone.
                </p>
              </div>
            </div>
