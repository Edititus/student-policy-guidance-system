...
export class AdminControllerDb {
  ...
  respondToQuery = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      try {
        ...
        if (q) {
          this.ragService
            .embedAdminQA(q.query, response, req.user.schoolId || q.schoolId || '', '')
            .catch((err: unknown) =>
  ...
  deleteQuery = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      await this.adminService.deleteQuery(queryId, req.user.role, req.user.schoolId)
  ...
  getAllQueries = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      const schoolId = req.user?.role === 'super_admin' ? undefined : req.user?.schoolId
      const { startDate, endDate, confidence, limit: limitStr, offset: offsetStr } = req.query
  ...
  getPendingStudents = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      const { users, total } = await this.adminService.getPendingStudents(schoolId, {
        limit,
        offset,
      })
    ...
    try {
      ...
      if (!['active', 'suspended', 'rejected'].includes(status)) {
        res
          ...
          .json({
            success: false,
            message: 'Invalid status. Must be active, suspended, or rejected',
          })
        return
      }
  ...
  createStudent = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      const effectiveSchoolId =
        req.user?.role === 'super_admin' ? schoolId || req.user?.schoolId : req.user?.schoolId
